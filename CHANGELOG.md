...
- タグ（`[0]...[/0]`）が原文と一致しない場合、段落を先頭 run にまとめるフォールバックが無警告で行われ、run ごとの書式（色分けなど）が失われていた。該当件数と uid を警告と結果欄に表示するように変更
- 書き戻しに失敗しても、前回成功時の結果がステップ4に残り、ダウンロードボタンから古いファイルを取得できてしまう問題を修正。書き戻しを試みた時点で前回の結果を破棄するように変更
- 処理中にボタンが無効化されず、連打で多重実行・多重ダウンロードが発生していた問題を修正

### Added

- 翻訳プロンプトを編集可能にし、内容を localStorage に保存（訳先の言語や文体を変更可能）。「プロンプトを既定に戻す」ボタンを追加
- 処理中のスピナーと進捗表示（PPTX 生成の進捗率を含む）

### Changed

- メッセージ（成功・警告・エラー）を画面下部固定の表示に変更。ページ最下部ではなく常に見える位置に出るようにし、`aria-live` を付与
- ステップ2のボタン順を変更し、主要導線である「プロンプト＋本文をコピー」を primary に変更
